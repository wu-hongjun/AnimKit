...
def orientBipedalJointStructure():
    ...
    joints = ls('*bind_joint', r=1, type='joint')
    endJoints = ls('*end_joint', lf=1, dag=1, type="joint")
    ...
    for j in joints:
        ...
        n = j.shortName()
        if (j.rx.isLocked() or j.ry.isLocked() or j.rz.isLocked()):
            print('Rotate locked. Skipping '+str(j))
            continue
        
        if (j in endJoints or skipJoint(n)):
            continue
        """
        try:
        """
...
def createFingerRotationPaths(simRotation = 45, sides = ['left', 'right'], fingers = ['thumb', 'index', 'middle', 'ring', 'pinky'], mainGrpName="FINGER_ROTATION_PATHS"):
    ...
    for side in sides:
        for finger in fingers:
            ...
            # Copy any finger blocks to the last chain simulation
            '''
            for j in chain:
                j = PyNode(j)
                for c in j.getChildren():
                    s = c.getShape()
                    if (isinstance(s, nt.Mesh)): print s
            '''
...
class blockModelHelper_GUI:
    ...
    def gui(self):
        ...
        refreshSceneRow = rowLayout(    p=sectionList,
                                        nc=2    );
        ...
        refreshScene = iconTextButton(  p=refreshSceneRow,
                                        i='refresh.png',
                                        ...
                                        width=0.6*self.buttonWidth,
                                        en=False,
                                        c=reloadFileCallback  )
                                        
        enableRefreshScene = checkBox(    p=refreshSceneRow,
                                          l='Enable Refresh Button',
                                          height=self.buttonHeight,
                                          width=0.4*self.buttonWidth    )
        enableRefreshScene.setChangeCommand(lambda x: refreshScene.setEnable(enableRefreshScene.getValue()))                     
                                        
        sectionList.attachForm(refreshSceneRow, "top", 0)
        ...
        sectionList.attachControl(skeletonFrame, "top", 0, refreshSceneRow)
        # END SKELETON HELPER #
