...
class Gestures(Network_Node):
    ...
    def __init__(self, side, limb, root, controlled_anims, attrs = [], mins = [], maxs = [], dvs = [], tx = [],ty = [],rz = [], anim = None):
        exists = False
        ...
        if exists == False:
            ...
            for c_anim in controlled_anims:
                ...
                if attrs:
                    ...
                    select(cl = True)
                    orienter = group(n=(c_anim + '_' + limb +'_SDK_orient'),em = True)
                    aligner([orienter,new_bone],c_anim,rotation = False)
                    parent(new_bone,orienter)
                    parent(orienter,c_anim.getParent())
            ...
            if ui != None:
                parent(ui,component_grp)
                connect_objs_to_node([ui],network,'ui')
    ...
    def get_ui(self):
        network = PyNode(self.network)
        return listConnections(network.ui)[0]
