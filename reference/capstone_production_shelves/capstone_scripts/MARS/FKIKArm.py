...
class FKIKArm(FKIKLimb):
    ...
    def __init__(self, side, limb, start, end, soft_IK = True, stretch = True, stretch_type = 'position', dv = 0, connect_to = None, area = None, rig = None, node = None):
        if node != None: self.node = node
        else: self.node = self.exist_check('FKIKArm',start,'start')
        
        if self.node == None:
            #Run Function
            FKIKLimb.__init__(self,'FKIKArm', side, limb, start = start, end = end)
            ...
            if soft_IK == True:
                soft_IKs, soft_IKH = self.setup_soft_IK(name,bones,IK_anim,PV_anim,dists[0],IK_pos,stretch = stretch)
