...
class FKIKQuadLeg(FKIKLimb):
    ...
    def __init__(self, side, limb, start, end, front_piv ,back_piv, inner_piv, outer_piv, soft_IK = True,
        ...
        else: self.node = self.exist_check('FKIKQuadLeg',start,'start')
        
        if self.node == None:
            #Run Function
            FKIKLimb.__init__(self,'FKIKQuadLeg', side, limb, start = start, end = end)
            ...
            IK_anim, IK_null = mu.create_anims(bones[3], name, suffix = 'IK_anim', end = False, type = 'sphere')
            ...
            balltoe_IKH, eff = pm.ikHandle(n = name + '_balltoe_IKH',sj = IKs[3], ee = IKs[4], sol = 'ikSCsolver')
            toe_IKH, eff = pm.ikHandle(n = name + '_toe_IKH',sj = IKs[4], ee = IKs[5], sol = 'ikSCsolver')
            all_IKHs = [IKH,foot_IKH,balltoe_IKH,toe_IKH]
            ...
            anims = FK_anims + [IK_anim] + [PV_anim] + roll_anims
            self.IK_roll_attribute_setup(name,IK_anim,IK_roll)
            
            ankle_anims, ankle_nulls ,ankle_IKs, ankle_IKH = self.ankle_IK_setup(name,IKs,IK_anim,PV_anim)
            complete_foot_null = pm.group(n = '{0}_foot_roll_grp'.format(name),em = True)
            mu.aligner(complete_foot_null,IK_anim)
            anims += [ankle_anims[0]]
            ...
            ##IK Setup
            pm.pointConstraint(IK_roll[-3],ankle_IKs[0],mo = True)
            pm.poleVectorConstraint(PV_anim,ankle_IKH)
            pc = pm.parentConstraint(IKs[0],ankle_IKH,mo = False)
            pm.delete(pc)
            pm.parentConstraint(ankle_nulls[1],IK_pos,mo = True)
            pm.parentConstraint(ankle_IKs[0],ankle_nulls[0],mo = True)
            
            pm.parentConstraint(IK_roll[-3],foot_IKH,mo = True)
            pm.parentConstraint(IK_roll[-2],balltoe_IKH,mo = True)
            pm.parentConstraint(IK_roll[-1],toe_IKH,mo = True)
            pm.parentConstraint(IK_pos,IKH)
            pm.poleVectorConstraint(PV_anim,IKH)
            
            #Stretch Setup
            locs, dists, dist_cons = self.set_distance_nodes(name,IKH, IKs, ankle_nulls[1], PV_anim)
            pm.parent(locs,[x.getParent() for x in dists],dist_grp)
            if stretch == True:
                FK_mds = mu.FK_stretch_setup(FK_anims[:3],FK_nulls,controls,stretch_type)
                IK_mds = self.IK_limb_stretch_setup(name,dists,IKs[:-3],IK_anim,stretch_type)
                self.FKIK_stretch_blend(switch, controls[:-3], FK_mds, IK_mds, stretch_type)
            ...
            if soft_IK == True: soft_IKs, soft_IKH = self.setup_soft_IK(name,bones,IK_anim,PV_anim,dists[0],IK_pos,area = ankle_nulls[1], stretch = stretch)
            IK_snaps = self.IK_anim_snaps(FK_anims[1:3], [PV_anim, IK_anim], 2, stretch)
            [pm.parent(x.getParent(),hidden_grp) for x in IK_snaps]
            
            #Parent Setup
            pm.parent(ankle_nulls[0],foot_null,complete_foot_null)
            pm.parent(complete_foot_null,IK_anim)
            pm.parent(controls[0], IKs[0], IKH, foot_IKH, toe_IKH, balltoe_IKH,ankle_IKH,ankle_IKs[0],IK_pos, dist_grp, hidden_grp)
            ...
            mu.lock_and_hide_attrs(ankle_anims,['translate','scale','visibility','radius'])
            mu.lock_and_hide_attrs(ankle_nulls,'all')
            pm.select(cl=True)
            ...
            mu.con_link(ankle_anims[0],self.node,'quad_anim')
            ...
            mu.con_link(complete_foot_null,self.node,'foot_null')
    ...
    def ankle_IK_setup(self,name,IKs,IK_anim,PV_anim):
        ankle_anims,ankle_nulls = mu.create_anims(IKs[2:4],name,suffix = 'quad_IK_anim',reverse = True)
        #pm.parent(ankle_nulls[0],IK_anim)
        ankle_IKs = mu.duplicate_chain(IKs[2:4],name,suffix = 'ankle_anim_IK',reverse = True)
        ankle_IKH, eff = pm.ikHandle(n = name + '_ankle_anim_IKH',sj = ankle_IKs[0], ee = ankle_IKs[1], sol = 'ikRPsolver')
        return ankle_anims, ankle_nulls ,ankle_IKs, ankle_IKH