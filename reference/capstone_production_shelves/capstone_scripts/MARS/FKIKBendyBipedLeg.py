...
class FKIKBendyBipedLeg(FKIKLimb):
    '''
    MANDATORY VARIABLES
        ...
        front_piv = string of the front pivot of the foot
    ...
    def __init__(self, side, limb, start, end, front_piv ,back_piv, inner_piv, outer_piv, upper_rolls, lower_rolls, soft_IK = True,
                 stretch = True, stretch_type = 'position', dv = 0, connect_to = None, area = None, rig = None, node = None):
        if node != None: self.node = node
        else: self.node = self.exist_check('FKIKBendyBipedLeg',start,'start')
        
        if self.node == None:
            #Run Function
            FKIKLimb.__init__(self,'FKIKBendyBipedLeg', side, limb, start = start, end = end)
            ...
            if soft_IK == True: soft_IKs, soft_IKH = self.setup_soft_IK(name,bones,IK_anim,PV_anim,dists[0],IK_pos,area = IK_roll[-3],stretch = stretch)
